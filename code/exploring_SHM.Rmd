...
mutations_above_threshold_by_clone_by_compartment <- count_mutations_above_threshold_by_clone(clone_freqs_by_tissue_and_cell_type,
                                                                                              mutation_freqs_within_clones_by_tissue_and_cell_type,
                                                                                              mutation_freq_threshold = mutation_freq_threshold) %>%
   mutate(compartment_cell_type = factor(compartment_cell_type, levels = c('naive','GC','PC','mem','nonnaive_IgD+B220+')), 
          group_controls_pooled = factor(group_controls_pooled, levels = group_controls_pooled_factor_levels),
          compartment_tissue = factor(compartment_tissue, levels = c('LN','spleen','BM'))) %>%
  filter(n_clone_seqs_in_compartment >= min_clone_size)
...
fraction_clones_with_1plus_mutations_by_compartment <-
  get_fraction_clones_with_n_mutations_above_threshold(mutations_above_threshold_by_clone_by_compartment,
                                                       n_mutations_threshold = 1, min_clone_size = min_clone_size) %>%
...
shared_mutations_in_LN_clones <-  mutations_above_threshold_by_clone_by_compartment %>% 
  ...
  group_by(group_controls_pooled, day, infection_status, compartment_cell_type, v_gene) %>%
  summarise(count_mutations_shared_by_clone_pairs(mutations_list = mutations_list)) %>%
  ungroup()

shared_mutations_in_LN_clones_pl <- shared_mutations_in_LN_clones %>%
  mutate(any_mutations_shared = shared_mutations > 0) %>%
  group_by(group_controls_pooled, day, infection_status, compartment_cell_type, any_mutations_shared) %>%
  count() %>%
  group_by(group_controls_pooled, day, infection_status, compartment_cell_type) %>%
  ...
  ylim(0,1) +
  theme(legend.position = 'top') +
  scale_x_continuous(breaks = as.integer(unique(shared_mutations_in_LN_clones$day))) +
...
shared_mutations_in_LN_clones %>%
  ...
  mutate(total_n_clone_pairs = sum(n),
         probability = n/total_n_clone_pairs) %>%
  filter(any_mutations_shared == T) %>%
  ggplot(aes(x = group_controls_pooled, y = probability, fill = group_controls_pooled)) +
  geom_col() +
  geom_text(aes(y = probability + 0.02, label = total_n_clone_pairs)) +
  facet_wrap('compartment_cell_type', nrow = 1) +
  xlab('group') +
  ylab('Probability that two clones (10+ seqs.) using IGHV14-4\nhave high-frequency mutations in common') +
  ylim(0,1.05) +
  theme(legend.position = 'none')
...
shared_mutations_in_LN_clones %>%
  ...
  facet_wrap('compartment_cell_type', nrow = 1) +
  xlab('group') +
  ylab('Probability that two clones (10+ seqs.) using IGHV1-82\nhave high-frequency mutations in common') +
  ylim(0,1.05) +
  theme(legend.position = 'none')

shared_mutations_in_LN_clones %>%
  filter(grepl('IGHV1-69', v_gene)) %>%
  mutate(any_mutations_shared = shared_mutations > 0) %>%
  group_by(group_controls_pooled, compartment_cell_type, any_mutations_shared) %>%
  count() %>%
  group_by(group_controls_pooled, compartment_cell_type) %>%
...
# Looking closely  at mutations driving some peaks in the above figures...

shared_mutations_in_LN_clones %>% filter(grepl('IGHV1-82', v_gene), compartment_cell_type == 'PC', group_controls_pooled == 'primary-16') %>%
  filter(shared_mutations > 0) %>%
  filter(grepl('F95Y', mutations_list_clone_i))

mutations_above_threshold_by_clone_by_compartment %>%
  filter(grepl('IGHV1-82', v_gene), compartment_cell_type == 'PC', compartment_tissue == 'LN', group_controls_pooled == 'primary-16',
         grepl('F95Y', mutations_list))



shared_mutations_in_LN_clones %>% filter(grepl('IGHV1-82', v_gene), compartment_cell_type == 'PC', group_controls_pooled == 'secondary-40') %>%
  filter(shared_mutations > 0) %>%
  filter(grepl('Y27F', mutations_list_clone_i), grepl('Y27F', mutations_list_clone_j))

mutations_above_threshold_by_clone_by_compartment %>%
  filter(grepl('IGHV1-82', v_gene), compartment_cell_type == 'PC', compartment_tissue == 'LN', group_controls_pooled == 'secondary-40',
         grepl('Y27F', mutations_list))


shared_mutations_in_LN_clones %>% filter(grepl('IGHV14-4', v_gene), compartment_cell_type == 'PC', group_controls_pooled == 'secondary-40') %>%
  filter(shared_mutations > 0) %>%
  filter(grepl('T91S', mutations_list_clone_i), grepl('T91S', mutations_list_clone_j))

mutations_above_threshold_by_clone_by_compartment %>%
  filter(grepl('IGHV14-4', v_gene), compartment_cell_type == 'PC', compartment_tissue == 'LN', group_controls_pooled == 'secondary-40',
         grepl('T91S', mutations_list))


shared_mutations_in_LN_clones %>% filter(grepl('IGHV14-4', v_gene), compartment_cell_type == 'mem', group_controls_pooled == 'secondary-56') %>%
  filter(shared_mutations > 0) %>%
  filter(grepl('Y32D', mutations_list_clone_i), grepl('Y32D', mutations_list_clone_j))

mutations_above_threshold_by_clone_by_compartment %>%
  filter(grepl('IGHV14-4', v_gene), compartment_cell_type == 'mem', compartment_tissue == 'LN', group_controls_pooled == 'secondary-56',
         grepl('Y32D', mutations_list)) 


shared_mutations_in_LN_clones %>% filter(grepl('IGHV1-69', v_gene), compartment_cell_type == 'PC', group_controls_pooled == 'secondary-40') %>%
  filter(shared_mutations > 0) %>%
  filter(grepl('Q43H;S56G;N59D;Q82R', mutations_list_clone_i), grepl('Q43H;S56G;N59D;Q82R', mutations_list_clone_j))

mutations_above_threshold_by_clone_by_compartment %>%
  filter(grepl('IGHV1-69', v_gene), compartment_cell_type == 'PC', compartment_tissue == 'LN', group_controls_pooled == 'secondary-40',
         grepl('Q43H;S56G;N59D;Q82R', mutations_list)) 
