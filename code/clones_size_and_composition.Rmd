...
# Clone frequencies in each cell type / tissue combination (not included in clone_info) is in a precomputed object
# NOTE: currently these cell type / tissue combination are relative to the total number of productive sequences
...
denominator <- 'prod_seqs'

#### NOTE ::: THIS CURRENTLY DOES NOT AFFECT THE clone_freqs OBJECTS. THOSE ONLY HAVE TOTAL PRODUCTIVE SEQUENCES CURRENTLY
...
How many clones does each mouse have? Mouse typically have tens of thousands of clones. including naive cells and singletons. They have thousands of clones with at least 10 sequences, hundreds of clones with at least 100 sequences, and tens of clones with at least a thousand sequence.
...
rank_size_plot <- clone_info %>%
  ...
  ggplot(aes(x = total_clone_rank, y = total_clone_seqs, group = mouse_id, color = infection_status)) +
  geom_line(alpha = 0.7) +
  scale_y_log10() +
  scale_x_log10() +
  theme(legend.position = 'none') +
...
rank_freq_plot <- clone_info %>%
  filter(total_clone_rank <= 1000) %>%
  ggplot(aes(x = total_clone_rank, y = clone_freq_in_mouse, group = mouse_id, color = infection_status)) +
  geom_line(alpha = 0.7) +
  scale_y_log10() +
  scale_x_log10() +
...
rank_size_plot_by_tissue <- clone_freqs_by_tissue %>%
  ...
  ggplot(aes(x = clone_rank_in_compartment, y = n_clone_seqs_in_compartment, group = mouse_id, color = infection_status)) +
  geom_line(alpha = 0.7) +
...
rank_freq_plot_by_tissue <- clone_freqs_by_tissue %>%
  filter(clone_rank_in_compartment <= 1000) %>%
  ggplot(aes(x = clone_rank_in_compartment, y = clone_freq_in_compartment, group = mouse_id, color = infection_status)) +
...
cumulative_curve_by_tissue_and_cell_type %>%
  ...
  ggplot(aes(x = clone_rank_in_compartment, y = cumulative_fraction_seqs, group = mouse_id,
             color = infection_status)) +
  #geom_vline(xintercept = c(10,100), linetype = 2) +
  #geom_hline(yintercept = c(1000, 100), linetype = 2) +
  geom_line(alpha = 0.7) +
...
clone_info %>%
  select(mouse_id, group_controls_pooled, clone_id, total_clone_seqs, BM_seqs, spleen_seqs,
         LN_seqs) %>%
...
clone_info %>%
  ...
  group_by(mouse_id, group_controls_pooled, size_filter) %>%
  dplyr::count() %>%
...
clone_info %>%
  filter(total_clone_seqs >= min_clone_size) %>%
  group_by(mouse_id, group_controls_pooled, biggest_cell_type_fraction > 0.9) %>%
  dplyr::count() %>%
  group_by(mouse_id, group_controls_pooled) %>%
  mutate(fraction = n/sum(n)) %>%
  filter(`biggest_cell_type_fraction > 0.9` == T) %>%
  ggplot(aes(x = group_controls_pooled, y = fraction)) +
  geom_boxplot(outlier.alpha = 0) +
  geom_point() +
  xlab('Group') +
