...
echo "#SBATCH --job-name=run_partis_day_${time_point}" >> $sbatch_file
echo '#SBATCH --output=out_err_files/run_partis_%A_%a.out' >> $sbatch_file
echo '#SBATCH --error=out_err_files/run_partis_%A_%a.err' >> $sbatch_file
...
echo output_file=../results/partis/${time_point}-'${SLURM_ARRAY_TASK_ID}_partis'.yaml >> $sbatch_file
...
echo else >> $sbatch_file
    ...
    # Run partis
    echo /project2/cobey/partis/bin/partis partition --n-procs 15 --species mouse --infname '$temp_fasta_file' --outfname '$output_file' --extra-annotation-columns regional_bounds:cdr3_seqs:seqs_aa:naive_seq_aa:consensus_seq:consensus_seq_aa >> $sbatch_file
